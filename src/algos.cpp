...
static int calculate_incremental_cost(const Graph &g1, const Graph &g2, const Mapping &mapping, const Vertex g1_vertex)
{
    ...
    g1.IterateNeighbours(
        [&](Vertex u) {
            ...
            // Calculate other way (if different)
            if (u != g1_vertex) {
...
static int CalculateHeuristic_(const Graph &g1, const Graph &g2, const State &state)
{
    ...
    for (Vertex v1 = 0; v1 < g1.GetVertices(); ++v1) {
        ...
        // Check all available vertices in G2
        for (Vertex v2 = 0; v2 < n2; ++v2) {
            if (state.used_mask.test(v2))
                continue;
            ...
            // Small optimization: if cost is 0, it's the best possible, break early
            if (min_cost == 0)
                break;
        }
        // If min_cost is still INT_MAX, it means no G2 vertices were available (shouldn't happen if sizes are correct)
        if (min_cost != INT_MAX) {
            h += min_cost;
        }
    }
...
std::vector<Mapping> AccurateAStar(const Graph &g1, const Graph &g2, const int k)
{
    ...
    while (!pq.empty()) {
        ...
        for (Vertex v2 = 0; v2 < n2; ++v2) {
            if (current.state.used_mask.test(v2))
                continue;
...
struct PrioArr {
    ...
    AStarState GetBest()
    {
        ...
        AStarState rv = std::move(table_[0]);

        for (size_t idx = 0; idx < used_ - 1; ++idx) {
            table_[idx] = std::move(table_[idx + 1]);
        }
        used_--;
    ...
    void Insert(AStarState &state)
    {
        ...
        for (size_t i = used_ - 1; i > insert_pos; --i) {
            table_[i] = std::move(table_[i - 1]);
        }

        table_[insert_pos] = std::move(state);
    }
...
struct MasterQueue {
    ...
    void Insert(const std::uint32_t idx, AStarState &state)
    {
...
NODISCARD std::vector<Mapping> ApproxAStar_(const Graph &g1, const Graph &g2, int k)
{
    ...
    while (!master_queue.IsEmpty()) {
        ...
        for (Vertex mapping_candidate = 0; mapping_candidate < n2; ++mapping_candidate) {
            if (best_state.state.used_mask.test(mapping_candidate))
                continue;
